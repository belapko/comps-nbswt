...
const ExtendedTabs: FC<IExtendedTabs> = ({ theme, tabs, setTabs }) => {
    ...
    useEffect(() => {
        ...
        setTabs && setTabs(tabs.filter((tab) => tab !== tabToClose));
    }, [close]);
        ...
        <div className={cn(styles.navbar, styles[theme])}>
            {tabs.map((tab, index) => (
                <Tab
                    ...
                    theme={theme}
                    onClick={tab.onClick}
                    isCanClosed={tab.isCanClosed}
                    menu={tab.menu}
                    setClose={setClose}
                    setWindowTitle={tab.setWindowTitle}
                    id={index}
                    isInWindowTitle={tab.isInWindowTitle}
                    isIconFirst={tab.isIconFirst}
                    colorsSettings={tab.colorsSettings}
                    active={tabs.indexOf(tab) === active}
                    setActive={() => setActive(tabs.indexOf(tab))}
                />
            ))}
        </div>
    );
